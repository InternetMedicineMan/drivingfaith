    ...
    public function handle(WebhookReceived $event): void
    {
        ...
        if ($event->payload['type'] === 'checkout.session.completed') {
            ...
            if ($checkoutSession['mode'] === 'payment') {
                ...
                $price = Price::where('stripe_id', $checkoutSession['metadata']['price'])->first();
                ...
                if ($price) {
                    ...
                    Log::info('Stripe order created for user: '.$user->id);
                } else {
                    // Log an error if the price is not found
                    Log::error('Price not found for Stripe checkout session: '.$checkoutSession['id']);
                }
            }
        }
        ...
        if ($event->payload['type'] === 'product.created' || $event->payload['type'] === 'product.updated') {
            ...
            Log::info('Product created/updated: '.$product->name);
        }
        ...
        if ($event->payload['type'] === 'price.created' || $event->payload['type'] === 'price.updated') {
            ...
            Log::info('Price created/updated: '.$price->stripe_id);
        }
    }
}
