...
<template>
    ...
    <AuthenticationCard>
        <template #actions>
            ...
            <Link :href="route('login')" class="inline-flex h-10 items-center rounded-md bg-[#305f49] px-4 font-medium text-[#fbf8ec] transition hover:bg-[#254938]">
                Sign in
            </Link>
        </template>
        ...
        <div class="mb-8 text-center">
            <h2 class="text-2xl font-bold text-[#24352c]">{{ $t('Create your account') }}</h2>
            <p class="mt-2 text-sm text-[#637466]">{{ $t('Start your journey with us today') }}</p>
        </div>
        ...
        <form @submit.prevent="submit" class="space-y-6">
            ...
            <div class="relative">
                ...
                <div class="relative flex justify-center text-sm">
                    <span class="bg-white px-4 text-base-content/60">{{ $t('Already have an account?') }}</span>
                </div>
            </div>

            <Link :href="route('login')" class="btn btn-outline btn-block border-[#305f49]/30 text-[#305f49] hover:border-[#305f49] hover:bg-[#305f49] hover:text-white">
                {{ $t('Sign in instead') }}
            </Link>
        </form>
    </AuthenticationCard>
